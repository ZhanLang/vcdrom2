...
protected:
	...
	SMatrix InitMatrix(double angle, SOUI::CPoint &center)
	{
	...
	void OnPaint(SOUI::IRenderTarget * pRT)
	{
		...
		SOUI::CPoint center( (m_rcItem.right - m_rcItem.left)/2 + m_rcItem.left, (m_rcItem.bottom - m_rcItem.top)/2 + m_rcItem.top);

		SMatrix form = InitMatrix( m_nTurn == TURN_RIGHT ?  nTmp : -nTmp, center);
		SOUI::CRect rcImg(SOUI::CPoint(0,0),m_pAnimImg->Size());
